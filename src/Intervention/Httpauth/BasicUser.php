...
class BasicUser implements UserInterface
{
    ...
    public function parse()
    {
        ...
        } elseif(array_key_exists('REDIRECT_HTTP_AUTHENTICATION', $_SERVER)) { // most other servers
  
          if(strpos(strtolower($_SERVER['REDIRECT_HTTP_AUTHENTICATION']), 'basic') === 0) {
    
            $userdata = explode(':', base64_decode(substr($_SERVER['REDIRECT_HTTP_AUTHENTICATION'], 6)));
            list($this->name, $this->password) = $userdata;
